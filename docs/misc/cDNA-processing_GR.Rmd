...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/P53_reporter_scan/data/gcf6502/results/mt20220902_reporter_activity_filt.csv", header = T) %>%
  filter(condition %in% c("MCF7_KO", "K562", "K562_Dex")) %>%
  filter(tf != "P53")
...
cDNA_df_cor_mcf <- cDNA_df %>%
  filter(tf == "GR", motif_id != "4_4_4_4", promoter != "random", str_detect(reporter_id, "random", negate = T)) %>%
  filter(str_detect(condition, "MCF7_WT")) %>%
  #filter(str_detect(sample_id, "MCF7_r1_gcf6412", negate = T)) %>%
  #filter(str_detect(sample_id, "gcf6881", negate = T)) %>%
  dplyr::select(reporter_id, reporter_activity_sample, condition, reporter_activity_condition, motif_id) %>%
  #mutate(reporter_activity_sample = log2(reporter_activity_sample)) %>%
  unique()
...
ggpairs(cDNA_df_cor_mcf,
               ...
               diag = list(continuous = function(data, mapping, ...) {
                   ...
                   theme_bw()})) +
  ggtitle(paste("Correlation Between Conditions")) +
...
x <- cDNA_df_cor_mcf %>% 
         filter(tf == "GR", motif_id != "4_4_4_4", promoter != "random") %>%
...
ggplot(x %>% 
       ...
       aes(x = MCF7_WT_DMSO, y = MCF7_WT_Nutlin, color = dif)) +
  geom_point() +
  scale_color_gradient(low = "#E6EEE8", high = colors_continous[2]) +
  geom_abline(linetype = "dashed") +
  facet_wrap(~motif_id)
...
ggplot(x %>% 
         filter(motif_id %in% c("0_0_0_0", "1_1_1_1", "2_2_2_2", "3_3_3_3")) %>% 
         filter(neg_ctrls == "No", positive_ctrl == "No"), 
       aes(x = MCF7_KO, y = MCF7_WT_Nutlin, color = dif)) +
  geom_point() +
  scale_color_gradient(low = "#E6EEE8", high = colors_continous[2]) +
  geom_abline(linetype = "dashed") +
  facet_wrap(~motif_id)
...
cDNA_df_cor_mcf2 <- cDNA_df %>%
  dplyr::select(reporter_id, reporter_activity_sample, sample_id) %>%
  unique() %>%
  spread(sample_id, reporter_activity_sample)
cDNA_df_cor_mcf2 <- merge(cDNA_df_cor_mcf2, ctrls)

ggplot(cDNA_df_cor_mcf2 %>%
         filter(tf == "GR", positive_ctrl == "No"),
       aes(x = MCF7_r1_gcf6502, y = MCF7_r2_gcf6502, color = neg_ctrls, alpha = 0.7)) +
  geom_point() +
  scale_color_manual(values = c(colors_continous[2], "black"))

ggplot(cDNA_df_cor_mcf2 %>%
         filter(tf == "GR", positive_ctrl == "No"),
       aes(x = MCF7_Nutlin_r1_gcf6502, y = MCF7_Nutlin_r2_gcf6502, color = neg_ctrls, alpha = 0.7)) +
  geom_point() +
  scale_color_manual(values = c(colors_continous[2], "black"))+
  theme_pubr(border = T)

ggpairs(cDNA_df_cor_mcf2 %>% dplyr::select(-reporter_id, -neg_ctrls, -tf, -promoter, -positive_ctrl, -motif_id),
                 upper = list(continuous = corColor),
               lower = list(continuous = function(data, mapping, ...) {
                   ggally_points(data = data, mapping = mapping, alpha = 0.1, size = 0.5) +
                   geom_abline(slope = 1, lty = "dashed", col = "red") +
...
cDNA_df_cor_mcf3 <- cDNA_df_cor_mcf2 %>%
  filter(tf == "GR" | ctrl_reporters == "Yes", positive_ctrl == "No")
cDNA_df_cor_mcf3 <- cDNA_df_cor_mcf3[-grep("P53_random[1-3]{1}_", cDNA_df_cor_mcf3$reporter_id),]

pmain <- ggplot(cDNA_df_cor_mcf3,
       aes(x = log2(MCF7_KO_r1_gcf6502), y = log2(MCF7_r1_gcf6502), color = ctrl_reporters)) +
...
xdens <- axis_canvas(pmain, axis = "x") +
  geom_density(data = cDNA_df_cor_mcf3, mapping = aes(x = MCF7_KO_r1_gcf6502, fill = ctrl_reporters),
              alpha = 0.7, size = 0.2) +
    scale_fill_manual(values = c(colors_continous[2], "black")) 
# Marginal densities along y axis
# Need to set coord_flip = TRUE, if you plan to use coord_flip()
ydens <- axis_canvas(pmain, axis = "y", coord_flip = TRUE)+
  geom_density(data = cDNA_df_cor_mcf3, aes(x = MCF7_r1_gcf6502, fill = ctrl_reporters),
                alpha = 0.7, size = 0.2)+
  coord_flip() +
  scale_fill_manual(values = c(colors_continous[2], "black")) 
...
ggplot(cDNA_df_cor_mcf2 %>%
           filter(tf == "GR" | ctrl_reporters == "Yes", positive_ctrl == "No"),
       aes(x = MCF7_r2_gcf6502, y = MCF7_r1_gcf6502, color = ctrl_reporters)) +
    geom_point() +
    scale_color_manual(values = c(colors_continous[2], "black"))+
    theme_pubr(border = T)
...
data <- cDNA_df %>%
           dplyr::select(reporter_activity_condition, positive_ctrl, promoter, condition, neg_ctrls, outlier, affinity_id, tf, condition, reporter_id) %>%
           filter(promoter != "random", tf == "GR", 
...
data <- cDNA_df %>%
  filter(neg_ctrls == "No", positive_ctrl == "No", promoter != "random", motif_id != "4_4_4_4", tf == "GR") %>%
...
data <- cDNA_df %>%
  filter(neg_ctrls == "Yes", positive_ctrl == "No",tf == "GR") %>%
  dplyr::select(tf, reporter_activity_condition, n_sites, motif_id, condition, reporter_id_2, background, promoter) %>% 
  unique() %>%
  na.omit()
...
data <- cDNA_df %>%
  filter(neg_ctrls == "No", positive_ctrl == "No", promoter != "random", motif_id != "4_4_4_4", spacing == 0, tf == "GR") %>%
  dplyr::select(tf, reporter_activity_condition, n_sites, motif_id, condition, reporter_id_2, background, promoter) %>% 
  unique()%>%
  spread(promoter, reporter_activity_condition) %>% 
  na.omit()
...
ggplot(cDNA_df %>%
         ...
         distinct(tf, reporter_activity_condition, spacing, condition, affinity_id, reporter_id),
       aes(x = spacing, y = reporter_activity_condition, color = condition, fill = condition)) +
  geom_hline(yintercept = 1, lty = "dashed", alpha = 0.1) +
  geom_point() +
  geom_smooth()+
  theme_pubr(border = T) +
...
ggplot(cDNA_df %>%
         filter(neg_ctrls == "No", promoter != "random", positive_ctrl == "No",
                str_detect(affinity_id, "[1]_"), position == 0, tf == "GR") %>%
...
ggplot(cDNA_df %>%
         filter(neg_ctrls == "No", promoter == "mCMV", positive_ctrl == "No", spacing == 7, tf == "GR",
                str_detect(condition, "K562"),
...
x <- cDNA_df %>%
         filter(neg_ctrls == "No", positive_ctrl == "No", promoter == "minP",
                str_detect(condition, "K562"),
                tf == "GR", spacing == 7, background == 1, position == 0, n_sites == 4, str_detect(affinity_id, "[1,2,3,4,5,6,7]{1}_"),
...
## Subselect reporters
cDNA_df_p53 <- cDNA_df %>%
  filter(neg_ctrls == "No", positive_ctrl == "No",tf == "GR",
         position == 0, promoter != "random",
         str_detect(affinity_id, "[1,3,5,7]_"), str_detect(condition, "K562")) %>%
...
## First for DMSO
cDNA_df_p53_dmso <- cDNA_df_p53[,-1] %>% filter(cDNA_df_p53$condition == "K562")
...
## Now for Nutlin
cDNA_df_p53_nutlin <- cDNA_df_p53[,-1] %>% filter(cDNA_df_p53$condition == "K562_Dex")
...
cDNA_df_cor_mcf <- cDNA_df %>%
  filter(str_detect(sample, "K562"), 
         tf == "GR", neg_ctrls == "No", positive_ctrl == "No") %>%
  dplyr::select(reporter_id, reporter_activity_sample, condition, spacing, n_sites, promoter, motif_id) %>%
  unique()

cDNA_df_cor_mcf$design <- "alt"
cDNA_df_cor_mcf$design[cDNA_df_cor_mcf$n_sites == 4 & cDNA_df_cor_mcf$spacing == 10] <- "common"

ggplot(cDNA_df_cor_mcf %>% 
         filter(motif_id != "4_4_4_4", promoter == "mCMV") %>%
         na.omit(), 
       aes(x = design, y = reporter_activity_sample, fill = condition)) +
...
data <- cDNA_df %>% 
         filter(neg_ctrls == "No", positive_ctrl == "No", promoter == "mCMV", 
                str_detect(condition, "K562"), str_detect(affinity_id, "[7-9]_"), 
                tf == "GR", spacing == 10) %>%
...
ggplot(cDNA_df %>%
         filter(neg_ctrls == "No", positive_ctrl == "No", promoter == "mCMV", position == 0, 
                str_detect(condition, "MCF7_WT"), str_detect(condition, "PFT", negate = T), spacing == 7,
                tf == "GR", str_detect(motif_id, "0_0_0_0|0_4_4_0|4_4_0_0|0_0_4_4|4_0_0_4")) %>%
...
ggplot(cDNA_df %>%
         filter(neg_ctrls == "No", positive_ctrl == "No", pDNA_library == "lib_2", promoter == "mCMV", position == 0, spacing == 7, 
                ...
                tf == "GR", str_detect(motif_id, "3_3_3_3|0_0_3_3|4_4_3_3|0_0_4_4|3_3_4_4|4_4_4_4|4_4_0_0|3_3_0_0|0_0_0_0|0_3_3_0|0_4_4_0|3_4_4_3")) %>%
         dplyr::select(reporter_activity_condition, condition, motif_id) %>%
         mutate(reporter_activity_condition = ave(reporter_activity_condition, motif_id, condition, FUN = function(x) mean(x))) %>%
         unique(),
       aes(x = motif_id, y = reporter_activity_condition, fill = condition))+
...
ggplot(cDNA_df %>%
         filter(neg_ctrls == "No", positive_ctrl == "No", pDNA_library == "lib_2", promoter == "minP", position == 0, spacing == 7, 
                ...
                tf == "GR", str_detect(motif_id, "3_3_3_3|0_0_3_3|3_3_0_0|0_0_0_0|0_3_3_0")) %>%
         dplyr::select(reporter_activity_condition, condition, motif_id) %>%
         mutate(reporter_activity_condition = ave(reporter_activity_condition, motif_id, condition, FUN = function(x) mean(x))) %>%
         unique(),
       aes(x = motif_id, y = reporter_activity_condition, fill = condition))+
...
x <- cDNA_df %>%
         filter(neg_ctrls == "No", positive_ctrl == "No", pDNA_library == "lib_2", promoter == "mCMV", position == 0, spacing == 7,
                str_detect(condition, "MCF7_WT_Nutlin"), str_detect(condition, "PFT", negate = T),
                tf == "GR", str_detect(motif_id, "2_3_3_3|3_3_3_2|1_1_1_0|0_1_1_1|2_2_2_1|1_2_2_2|2_2_3_3|3_3_2_2|1_1_2_2|2_2_1_1|1_1_0_0|0_0_1_1")) %>%
...
ggplot(cDNA_df_2 %>%
          filter(neg_ctrls == "No", positive_ctrl == "No",promoter == "mCMV", position == 0, spacing == 7,
                str_detect(condition, "MCF7_WT_Nutlin"), str_detect(condition, "PFT", negate = T),
                tf == "GR", str_detect(positioning, "high")) %>%
...
x <- cDNA_df_cor_mcf %>% 
         filter(tf == "GR", motif_id != "4_4_4_4", promoter != "random", neg_ctrls == "No") %>%
...
<!-- cDNA_df_p53 <- cDNA_df %>% -->
<!--   filter(neg_ctrls == "No", positive_ctrl == "No", pDNA_library == "lib_2", tf == "GR", -->
...
<!--          str_detect(condition, "MCF7_WT"), str_detect(condition, "PFT", negate = T), tf == "GR") %>% -->
