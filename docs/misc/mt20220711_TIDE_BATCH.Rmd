...
```{r functions}
import <- function(control_file, experimental_file, guide, seqstart, seqend, array_length, rg1 = NA, rg2 = NA) {
  ##Arguments: 
  ...
  ## array_length (numeric): range of basepair shifts (indels) to be analyzed, both positive and negative
  ...
  if(seqstart>(breaksite-array_length)){
  ...
  if((nchar(sequence_ctr) < ((breaksite-array_length)-seqstart)) | (nchar(sequence_mut) < ((breaksite-array_length)-seqstart)))
  {
    stop(paste("(one of) the sequence run(s) is too short for proper alignment"))}
  
  seq_ctr <- substr(sequence_ctr, seqstart, breaksite-array_length)
  seq_mut <- substr(sequence_mut, seqstart, breaksite-array_length)
  align_seq <- pairwiseAlignment(pattern = seq_mut, subject = seq_ctr, type = "local")
  if(align_seq@score<10){
  ...
  rg1<-ifelse(is.na(rg1), breaksite+array_length+5, rg1)
  rg2<-ifelse(is.na(rg2), seqend-array_length-5, rg2)
  
  #check if rg1 and rg2 are within meaningful range: 
  if(rg1< breaksite+array_length+5) {
    rg1<- breaksite+array_length+5
  ...
  if(rg2 > seqend-array_length-5) {
    rg2<- seqend-array_length-5
...
quality <- function(import) {
  ...
  ## import$array_length = which basepair shifts (indels) you want to know the percentage of.
  ##   Note: import$array_length is the number to one direction, in the calculation it determines the shift to both direction (deletion & insertion) 
...
decomposition <- function(import, plottitle,  p.threshold = 0.0001) {     
  ## All the arguments are generated in the function 'TIDE_import', except p.threshold
  ## import$ctr = peakheigths of the control sample (e.g. transfected CRISPR without guide seq)
  ## import$mut = peakheigths of the sample that have had a DSB/repair
  ## import$array_length = size range of indels to be considered in the decomposition.
  ##   Note: calculation is always done in both directions, i.e. for both deletions and insertions of sizes 0:array_length 
  ...
  shiftrange<-c(-0: import$array_length) 
  ...
  {#simulate sequencing peak data for all hypothesized indels from control peaks:
    sim <- matrix(NA, nrow=import$rg2-import$rg1+1, ncol=import$array_length+1)
  ...
  #standard error:
  se <- sqrt(diag((sum((NNFIT$fitted-I_vec)^2)/(nrow(I_matrix)-(import$array_length*2+1)))*solve(t(I_matrix)%*%I_matrix)));
  ...
  comper<-100*(NNFIT$x/sum(NNFIT$x))
  
  
  COL <- c("#373930", "#433C38", "#4C4A45", "#525D51", "#A1B0AB")
  plttit <- plottitle
  #plot decomposition graph              
  bp <- barplot(comper,
                col=COL, 
                border = COL, 
                names.arg = reporter_names,
                ylab="% of sequences",  
                xlab = c("TF reporters"),
                ylim=c(0, 100), 
                main = plttit)
  ...
  text(bp, (comper+3), as.character(((round(comper,1)))))
  ...
  # eff <- round((Rsq*100) - comper[import$array_length+1],1)
...
insertion <- function(import, decomposition) {   
  ...
  ## import$seqend = last bp to be included in analysis (will be automatically adjusted if reads are shorter, see below)
  ## import$array_length = which basepair shifts (indels) you want to know the percentage of.
  ##   Note: array_length is the number to one direction, in the calculation it determines the shift to both direction (deletion & insertion) 
  ## import$rg1/import$rg2 = the sequence trace that is used for decomposition
  ##   Note: rg1&rg2 should be always after the breaksite
  ## import$offset_mut = the offset that seuquence trace of the sample has with repect to the control sequence trace.
  ...
  {
    SigShift <- which(decomposition$pv<decomposition$p.threshold)-import$array_length-1; 
...
```{r}
ref.file <- "/DATA/usr/m.trauernicht/projects/P53_reporter_scan/data/TIDE/E1709_20220711/mt20220711_seq.txt"
output.file <- "/DATA/usr/m.trauernicht/projects/P53_reporter_scan/data/TIDE/E1709_20220711/mt20220711_TIDE.pdf"
folder <- "/DATA/usr/m.trauernicht/projects/P53_reporter_scan/data/TIDE/E1709_20220711/"
reporter_names <- c("sp1", "+1", "random", "+3", "low_conc", "+5", "high_conc", "+7", "pos_ctrl", "+9")
seqstart = 2
seqend = 300
array_length = 9
```
...
pdf(output.file, width = 11, height = 10)
par(mfrow=c(3,3))
plot_list = apply(table, 1, function(x){
  print(x[1])
  x[1:2] = paste0(folder, x[1:2])
  imp = import(x[1], x[2], x[3], seqstart, seqend, array_length)
  #p_qual = quality(imp)
...
## Export percentages
```{r}
percentage_df <- data.frame("condition" = unique(table$V4), "activity" = "", x = 1:length(unique(table$V4)))
percentage_df <- merge(percentage_df, data.frame('y' = 1:(array_length+1), 'TF' = unique(reporter_names)))


for (i in 1:length(plot_list)) {
  for (j in 1:(array_length+1)) {
  percentage_df$activity[percentage_df$x == i & percentage_df$y == j] <- plot_list[[i]]$NNFIT$x[j]/sum(plot_list[[i]]$NNFIT$x)
  }
}

percentage_df <- percentage_df %>%
  distinct(condition, activity, TF)

write.csv2(percentage_df, paste(folder, "activity_summary.csv", sep = ""))
