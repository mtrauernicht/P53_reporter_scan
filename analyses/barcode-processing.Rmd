---
title: "Barcode processing - TP53 Reporter Scan"
author: 
...
### Aim
pMT06 (the pDNA library) was transfected into MCF7 cells that are either TP53 proficient or TP53-KO - 24 hours later mRNA was isolated and barcodes were quantified by sequencing together with pMT06 pDNA counts. This was done in three independent replicates.
...
metadata_df <- read_csv2("/DATA/usr/m.trauernicht/projects/P53_reporter_scan/data/mt20230202_metadata.csv") %>%
...
pDNA_bias <- data.frame("sample_id" = unique(bc_df_2$sample_id[grep("gcf6502|gcf6881", bc_df_2$sample_id)]),
...
pDNA_lib2 <- bc_df[grep("pMT06", bc_df$sample),] %>% 
