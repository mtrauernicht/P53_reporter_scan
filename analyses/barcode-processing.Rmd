...
metadata_df <- read_csv2("/DATA/usr/m.trauernicht/projects/P53_reporter_scan/data/mt20220530_metadata_all_experiments.csv") %>%
  na.omit() %>%
  filter(library == "new") %>%
  filter(condition %in% c("MCF7", "MCF7_Nutlin", "MCF7_KO", "MCF7_KO_Nutlin", "pMT06"))
...
metadata <- bc_df_filt %>%
  distinct(condition, replicate, gcf)
...
pDNA_bias <- data.frame("sample_id" = unique(bc_df_2$sample_id[grep("gcf6412|gcf6502|gcf6881", bc_df_2$sample_id)]),
...
pDNA_bias <- pDNA_bias %>%
  filter(as.numeric(cor) <= 0.6 | str_detect(sample_id, "pMT06")) 
...
pDNA_lib1 <- bc_df[grep("pMT06", bc_df$sample),] %>% 
