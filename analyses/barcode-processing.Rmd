...
metadata_df <- read_csv2("/DATA/usr/m.trauernicht/projects/P53_reporter_scan/data/mt20220530_metadata_all_experiments.csv") %>%
  na.omit() %>%
  filter(library == "new") %>%
  filter(condition %in% c("MCF7", "MCF7_Nutlin", "MCF7_KO", "MCF7_KO_Nutlin", "pMT06", "K562", "K562_Dex"))
...
bc_df_cor <- bc_df_filt %>%
  dplyr::select(rpm, barcode, sample_id)
...
metadata <- bc_df_filt %>%
  distinct(condition, replicate, gcf, sample_id)
...
p <- pca(bc_df_cor2, metadata = metadata %>%
           column_to_rownames("sample_id"), removeVar = 0.1)
biplot(p, 
       colby = "condition", 
...
plot_ly(bc_df_reads[bc_df_reads$gcf == "gcf6502",] %>% dplyr::select(sample, seq_sum) %>% unique(), x = ~sample, y = ~seq_sum, type = 'bar',
             marker = list(color = '#D6D5C9',
                           line = list(color = 'rgb(8,48,107)', width = 1.5))) %>% 
...
plot_ly(bc_df_reads[bc_df_reads$gcf == "gcf6881",] %>% dplyr::select(sample, seq_sum) %>% unique(), x = ~sample, y = ~seq_sum, type = 'bar',
             marker = list(color = '#D6D5C9',
                           line = list(color = 'rgb(8,48,107)', width = 1.5))) %>% 
...
pDNA <- data.frame("pDNA_lib2" = bc_df$rpm[bc_df$sample_id == "pMT06_r1_gcf6412"],
                   "barcode"= bc_df$barcode[bc_df$sample_id == "pMT06_r1_gcf6412"], stringsAsFactors=FALSE)
...
ggplot(bc_df_2 %>%
         filter(str_detect(sample_id, "gcf6412")), aes(x = pDNA_lib2, y = rpm)) +
  geom_bin2d(bins = 100)+
  xlim(0,1000) +
  ylim(0,1000)+
  theme_bw()+
...
ggplot(bc_df_2 %>%
         filter(str_detect(sample_id, "gcf6992")), aes(x = pDNA_lib3, y = rpm)) +
  geom_bin2d(bins = 100)+
  xlim(0,1000) +
  ylim(0,5000)+
  theme_bw()+
...
pDNA_bias <- data.frame("sample_id" = unique(bc_df_2$sample_id[grep("gcf6412|gcf6502|gcf6881|gcf6992", bc_df_2$sample_id)]),
                        "cor" = "", stringsAsFactors = F)
for (i in unique(pDNA_bias$sample_id)) {
  pDNA_bias$cor[pDNA_bias$sample_id == i] <- cor(bc_df_2$rpm[bc_df_2$pDNA_lib2 >= 250 & bc_df_2$sample_id == i], 
                                                 bc_df_2$pDNA_lib2[bc_df_2$pDNA_lib2 >= 250 & bc_df_2$sample_id == i], 
...
bc_df <- bc_df %>%
  filter(sample_id %in% c(pDNA_bias$sample, "pMT06_r1_gcf6412"))
...
pDNA_lib1 <- bc_df[grep("pMT06_r1", bc_df$sample),] %>% 
  dplyr::select(barcode,"pDNA_counts_rpm_gcf6412" = rpm, gcf)

pDNA_lib2 <- bc_df[grep("pMT06_r2", bc_df$sample),] %>% 
...
# pDNA <- pDNA_lib1 %>%
#   rowwise() %>%
#   mutate(pDNA_counts_rpm = mean(c(pMT06_r1, pMT06_r2), na.rm = T)) %>%
#   distinct(barcode, pDNA_counts_rpm)


bc_df <- merge(pDNA_lib1, bc_df, by = c("barcode", "reporter_id", "gcf"), all = T)
bc_df <- merge(pDNA_lib2, bc_df, by = c("reporter_id", "barcode"), all = T)
...
bc_df$activity[bc_df$gcf == "gcf6412"] <- bc_df$rpm[bc_df$gcf == "gcf6412"]  / bc_df$pDNA_counts_rpm_gcf6412[bc_df$gcf == "gcf6412"] 
...
## Combine replicates of normalized data in 3 different columns
bc_df_rep <- bc_df[!is.na(bc_df$tf),] %>% 
  dplyr::select(replicate, activity, barcode, gcf, condition) %>% 
  mutate(sample_id = paste(replicate, gcf, sep = "_")) %>%
  distinct(condition, barcode, activity, sample_id) %>%
  spread(sample_id, activity)
...
bc_df_2 <- bc_df %>%
  filter((pDNA_counts_rpm_gcf6412 > 3 & gcf == "gcf6412") | (pDNA_counts_rpm > 3 & gcf != "gcf6412") )
...
# bc_df_cDNA_filt <- bc_df_cDNA %>%
#   mutate(outlier = "No")
# bc_df_cDNA_filt$outlier[bc_df_cDNA_filt$reporter_activity > 3 & bc_df_cDNA_filt$reporter_activity_sd >= (bc_df_cDNA_filt$reporter_activity)] <- "Yes"
# 
# ## Plot the fraction of outliers per sample
# n_outlier <- bc_df_cDNA_filt %>%
#   filter(reporter_activity > 0.5) %>%
#   dplyr::select(outlier, sample_id, reporter_id) %>%
#   filter(outlier == "Yes") %>%
#   unique() %>% 
#   group_by(sample_id) %>%
...
#           dplyr::select(n_outlier, sample_id) %>%
#           unique(),
#         x = ~sample_id, y = ~n_outlier, type = 'bar',
#              marker = list(color = '#D6D5C9',
#                            line = list(color = 'rgb(8,48,107)', width = 1.5))) %>% 
