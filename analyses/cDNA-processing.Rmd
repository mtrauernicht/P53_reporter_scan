...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE_deep_scan_trp53_gr/data/gcf6502/results/mt20210701_reporter_activity_filt.csv", header = T) 
...
cDNA_df$reporter_activity_condition <- ave(cDNA_df$reporter_activity_norm, cDNA_df$condition, cDNA_df$reporter_id, 
...
ggplot(cDNA_df %>%
           dplyr::select(reporter_activity_sample, positive_ctrl, promoter, sample, neg_ctrls, outlier, affinity_id, tf, gcf, condition) %>%
