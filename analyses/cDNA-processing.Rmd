...
cDNA_df_neg <- cDNA_df %>%
  ...
  unique() %>%
  mutate(reporter_activity = ave(reporter_activity, sample_id, promoter, tf, FUN = mean)) %>%
...
#   unique() %>%
#   dplyr::group_by(sample_id, promoter) %>%
...
ggplot(cDNA_df %>%
         ...
         mutate(reporter_activity_spacing = ave(reporter_activity_condition, affinity_id, condition, spacing, FUN = function(x) mean(x))) %>%
         mutate(spacing_transf = cos(2 * pi * ((spacing -1) / 10.5))) %>%
         dplyr::group_by(condition, affinity_id) %>%
...
## Fitting the model
x <- lm(log2(reporter_activity) ~ spacing_degree_transf  + promoter*background + affinity_id, cDNA_df_p53_nutlin)
...
## Fitting the model
y <- lm(log2(reporter_activity) ~ spacing_degree_transf  + promoter*background + affinity_id, cDNA_df_p53_dmso)
