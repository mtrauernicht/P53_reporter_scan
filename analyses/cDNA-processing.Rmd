...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/P53_reporter_scan/data/gcf6502/results/mt20230207_reporter_activity_filt.csv", header = T)
...
cor(compare_df$gcf6502_r1, compare_df$gcf6881_r3, method = "spearman", use = "pairwise.complete.obs")
cor(compare_df$gcf6881_r3, compare_df$gcf6502_r2, method = "spearman", use = "pairwise.complete.obs")
...
## Figure 4C: reporter activity distribution - only random reporters
ggplot(cDNA_df %>%
         filter(condition %in% c("MCF7_WT_DMSO","MCF7_WT_Nutlin")) %>%
...
## Figure 4D: impact of background sequence
ggplot(cDNA_df %>% 
         filter(condition %in% c("MCF7_WT_DMSO","MCF7_WT_Nutlin"))%>%
...
## Figure 4E: correlate minP and mCMV reporter activities
data <- cDNA_df %>%
...
# Figure 4F: reporters with 0bp spacing
data <- cDNA_df %>%
