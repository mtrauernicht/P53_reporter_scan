...
cDNA_df <- merge(cDNA_df, reporter_affinity_p53)
...
ggplot(cDNA_df %>%
         ...
         filter(positive_ctrl == "Yes") %>%
         unique(), 
       aes(x = condition, y = reporter_activity, alpha = 0.4)) + 
  geom_quasirandom() + 
...
ggplot(cDNA_df %>%
         filter(outlier == "No", neg_ctrls == "No", promoter == "mCMV", positive_ctrl == "No", 
                str_detect(condition, "MCF7-WT"), str_detect(affinity_id, "[1-3]_"), spacing == 7) %>%
  ...
  geom_quasirandom(dodge.width = .75, alpha = .4) +
  geom_boxplot(position = position_dodge(.75), alpha = .4)+
  scale_color_manual(values = colors)+
  theme_bw() +
  labs(title = "reporter activity per background")+
  xlab("background sequence")+
...
ggplot(cDNA_df %>%
         filter(outlier == "No", neg_ctrls == "No", promoter == "mCMV", positive_ctrl == "No", spacing == 7, 
                str_detect(condition, "MCF")) %>%
         dplyr::select(reporter_activity_condition, condition, SumAffinity) %>%
         unique() %>%
       ...
       aes(x = SumAffinity, y = reporter_activity_condition))+
  geom_point(alpha = .4) +
...
ggplot(cDNA_df %>%
         ...
         group_by(condition) %>%
         mutate(SumAffinity = 1:length(SumAffinity)),
       aes(x = SumAffinity, y = reporter_activity_condition, fill = condition, color = condition))+
  geom_smooth(method = "gam", alpha = 0.4)+
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))+
  scale_color_manual(values = c("#2D3047", "#123139", "#FF9B71", "#F1DBA9")) +
  scale_fill_manual(values = c("#2D3047", "#123139", "#FF9B71", "#F1DBA9")) +
  ylab("reporter activity (a.u.)")+
  xlab("reporter affinity rank (increasing)")+
  labs(title = "reporter activity per affinity")
...
ggplot(cDNA_df %>%
         filter(outlier == "No", neg_ctrls == "No", promoter == "mCMV", positive_ctrl == "No", spacing == 7, 
                str_detect(condition, "MCF")) %>%
         dplyr::select(reporter_activity_condition, condition, reporter_affinity) %>%
         unique() %>%
         mutate(reporter_activity_condition = ave(reporter_activity_condition, condition, reporter_affinity,
                                                  FUN = function(x) mean(x))) %>%
         unique() %>%
         arrange(reporter_affinity) %>%
         group_by(condition) %>%
         mutate(reporter_affinity = 1:length(reporter_affinity)),
       aes(x = reporter_affinity, y = reporter_activity_condition))+
  geom_point(alpha = .4) +
  geom_smooth(method = "gam", fill = "#2D3047", color = "#2D3047", alpha = 0.4)+
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))+
...
ggplot(cDNA_df %>% 
         ...
         dplyr::select(cum_affinity, condition, reporter_id, reporter_activity_condition) %>%
