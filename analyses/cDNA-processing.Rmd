...
## Figure S2A: Activity in the KO condition
compare_df <- cDNA_df %>%
...
ggplot(cDNA_df %>%
         ...
         unique() %>%
         mutate(reporter_activity_spacing = ave(reporter_activity_condition, affinity_id, condition, spacing, FUN = function(x) mean(x))) %>%
         mutate(spacing_transf = cos(2 * pi * ((spacing -1) / 10.5))) %>%
         unique(), 
       aes(x = spacing_transf, y = reporter_activity_spacing, color = condition, fill = condition)) +
  geom_point() +
  geom_smooth()+
  theme_pubr(border = T) +
  ggtitle("reporter activity per spacing")+
...
ggplot(cDNA_df %>%
         ...
         mutate(reporter_activity_spacing = ave(reporter_activity_condition, affinity_id, condition, spacing, FUN = function(x) mean(x))) %>%
         mutate(spacing_transf = cos(2 * pi * ((spacing -1) / 10.5))) %>%
         group_by(condition, affinity_id) %>%
         mutate(predicted_act = spacing_transf * max(reporter_activity_spacing)) %>%
         ungroup() %>%
         unique(), 
       aes(color = condition, fill = condition)) +
  geom_point(aes(x = spacing, y = reporter_activity_spacing)) +
  geom_line(aes(x = spacing, y = predicted_act)) +
  theme_pubr(border = T) +
  ggtitle("reporter activity per spacing")+
  xlab("spacing length between motifs (bp)")+
  ylab("reporter activity (a.u.)")+
...
ggplot(cDNA_df %>%
         ...
         unique() %>%
         mutate(reporter_activity_spacing = ave(reporter_activity_condition, affinity_id, condition, spacing, FUN = function(x) mean(x))) %>%
         mutate(spacing_transf = 2 * pi * ((spacing -1) / 10.5)) %>%
         unique(), 
       aes(x = spacing_transf, y = reporter_activity_spacing, color = condition, fill = condition)) +
  geom_point() +
  geom_smooth()+
...
```{r out.width= "100%", fig.align= "center", echo=FALSE, warning= FALSE, message = FALSE}
## Figure 4A: reporter activity distribution - compare promoters
ggplot(cDNA_df %>%
...
## Figure S4A: minimal promoters background activity
data <- cDNA_df %>%
...
## Figure 4B: impact of background sequence
ggplot(cDNA_df %>% 
         filter(condition %in% c("MCF7_WT_DMSO","MCF7_WT_Nutlin"))%>%
...
## Figure 4C: activity as a function of spacer length per minimal promoter and spacer sequence
ggplot(cDNA_df %>%
         filter(neg_ctrls == "No", promoter != "random", positive_ctrl == "No",
                str_detect(affinity_id, "[7]{1}_"), str_detect(affinity_id, "11_high_mid", negate = T),
                position == 0, tf == "P53") %>%
...
## Figure S4B: reporter activity distribution - only random reporters
ggplot(cDNA_df %>%
         filter(condition %in% c("MCF7_WT_DMSO","MCF7_WT_Nutlin")) %>%
...
## Figure S4C: correlate minP and mCMV reporter activities
data <- cDNA_df %>%
...
# Figure S4D: reporters with 0bp spacing
data <- cDNA_df %>%
...
## Figure 5: Identify best reporters

Aim: What are the best reporters? Can we explain their design using a linear model? Are they better than commercial P53 reporters?


```{r out.width= "100%", fig.align= "center", echo=FALSE, warning= FALSE, message = FALSE}


## Figure 5A: Show that some reporters are superior to commercial reporters
compare_df <- cDNA_df %>%
...
ggplot(compare_df %>%
  ...
  geom_abline(lty = 2) +
  geom_point(shape = 21, aes(fill = dif, color = positive_ctrl), size = 2) +
  ...
  scale_fill_gradient2(low = "#DBE6DD", high = colors_continous[3],  limits = c(0,5), oob = squish)



## Figure 5C: Line-plot comparing sensitivities of my reporters to positive controls
...
cDNA_df_p53 <- cDNA_df_p53 %>%
  mutate(spacing_degree = 2* pi * ((spacing - 1) / 10.5)) %>%
...
cDNA_df_p53 <- cDNA_df_p53 %>%
  mutate(promoter_background = paste(promoter, background, sep = "_")) %>%
  mutate(promoter_background = as.factor(promoter_background))

cDNA_df_p53$promoter_background <- relevel(cDNA_df_p53$promoter_background, ref = "minP_1")

## Fitting the model
x <- lm(log2(MCF7_WT_Nutlin) ~ spacing_degree_transf  + promoter_background + affinity_id, cDNA_df_p53)
...
weight_intercepts <- data.frame("feature" = c("1_high_only", "promoter_backgroundminP_1"), "weight" = c(0, 0))
...
ggplot(cDNA_df_p53 %>%
       ...
       aes(x = log2(MCF7_WT_Nutlin), y = reporter_activity_predicted, color = top_6)) +
  geom_abline(lty = 2) +
  geom_smooth(color = "black", method = "lm") +
  geom_point(size = 2) +
  scale_color_manual(values = c("notop" = "black", "top" = "red")) +
  xlab("Measured reporter activity") +
  ylab("Predicted reporter activity") +
  theme(legend.position = "none")

level_order <- rev(c("spacing_degree_transf", "1_high_only", "3_med_only", "7_very-low_only", "minP_1", "minP_2", "minP_3", "mCMV_1", "mCMV_2", "mCMV_3"))
