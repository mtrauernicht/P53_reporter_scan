...
cDNA_df_p53 <- cDNA_df %>%
  ...
  mutate(MCF7_WT_Nutlin = log2(MCF7_WT_Nutlin)) %>%
...
## Fitting the model
x <- lm(dif ~ n_sites + spacing + highest_dist + motif_dist, cDNA_df_p53_dmso)
...
names(weight) <- c("feature", "weight")
references <- data.frame("feature" = c("spacing7", "n_sites1"), 
                         "weight" = c(0,0))
...
## Fitting the model
x <- lm(MCF7_WT_Nutlin ~ n_sites + spacing + highest_dist + motif_dist, cDNA_df_p53_nutlin)
...
level_order <- c("spacing0", "spacing1", "spacing2", "spacing3", "spacing4", "spacing5", "spacing6", "spacing7", "spacing8", "spacing9", "spacing10",
                 "n_sites1", "n_sites2", "n_sites3", "n_sites4", "highest_dist1", "motif_dist")

ggplot(weight %>% filter(condition == "Nutlin"), 
       aes(x = factor(feature, level = level_order), y = weight, fill = cond)) +
