...
ggplot(cDNA_df_p53_all %>%
         distinct(stimulation, reporter_activity_predicted, affinity_pos1, reporter_id) %>%
         pivot_wider(names_from = "stimulation", values_from = "reporter_activity_predicted"),
       aes(x = DMSO_mean, y = Nutlin_mean, color = affinity_pos1
           #, 
           #color = top_6
           )) +
  geom_abline(lty = 2) +
...
## Linear model parameter sweep to systematically identify best reporters
```{r}
## Nutlin condition
cDNA_df_p53_nutlin <- cDNA_df_p53 %>% filter(stimulation == "Nutlin_mean")
...
prediction <- merge(prediction,y, all = T) %>% 
  dplyr::select(-id)
cDNA_df_p53_nutlin <- merge(cDNA_df_p53_nutlin, prediction) %>%
  distinct(reporter_id, reporter_activity_predicted, stimulation)
...
cDNA_df_p53_all_wide <- cDNA_df_p53_all %>%
  pivot_wider(names_from = "stimulation", values_from = "reporter_activity_predicted")

ggplot(cDNA_df_p53_all_wide,
       aes(x = DMSO_mean, y = Nutlin_mean)) +
  geom_point()
