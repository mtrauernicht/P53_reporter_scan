...
pDNA_seq_bc <- pDNA_seq %>% select(barcode, rpm) %>% unique() %>% mutate(id = "pDNA")
...
pDNA_seq_bc <- pDNA_seq %>% select(barcode, rpm) %>% unique() 
...
pDNA_seq_seq <- pDNA_seq %>% select(sequence,rpm) %>% unique()
...
# mismatch_df <- pDNA_seq_seq[pDNA_seq_seq$match == F,] %>% select(barcode, number) %>% unique()
# mismatch_df <- ddply(mismatch_df,~barcode, summarise, number = sum(number))
# 
# match_df <- pDNA_seq_seq[pDNA_seq_seq$match == T,]  %>% select(barcode, number) %>% unique()
