...
ref_seq <- ReadFasta("/DATA/usr/m.trauernicht/projects/SuRE_deep_scan_trp53_gr/data/output/mt20200619_oligo_pool_deep.fasta")
...
pDNA_seq <- read.delim2("/DATA/usr/m.trauernicht/projects/SuRE_deep_scan_trp53_gr/pDNA_insert_seq/processed/pDNA-seq-counts.txt", col.names = c("sequence", "number"))
...
## Filter out low & noisy barcode reads
```{r}
# Filter pDNA data - we should have at least 25 pDNA counts - we want to be sure that we're looking at relevant things
pDNA_seq <- pDNA_seq[pDNA_seq$number >= 25,]
...
matching_df$type[log2(matching_df$ratio) < 2 & log2(matching_df$ratio) >= -2] <- "unclear matching"
matching_df$type[log2(matching_df$ratio) < -2] <- "incorrect matching"
