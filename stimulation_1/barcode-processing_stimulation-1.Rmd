---
title: "Barcode processing - Deep P53/GR scan - stimulation 1"
...
pDNA_insert_counts <- read.csv2("/DATA/usr/m.trauernicht/projects/SuRE_deep_scan_trp53_gr/data/pDNA_insert_seq/bc_counts.csv", stringsAsFactors = F) %>% dplyr::select(-X)
pDNA <- data.frame("pDNA" = bc_df$rpm[bc_df$condition == "pMT06_pDNA_rep1"],
                   "barcode"= bc_df$barcode[bc_df$condition == "pMT06_pDNA_rep1"],
                   "tf" = bc_df$tf[bc_df$condition == "pMT06_pDNA_rep1"], stringsAsFactors=FALSE)
...
pDNA_fraction <- pDNA_fraction %>% 
  ...
  dplyr::select(-pMT06_pDNA_rep1, -pMT06_pDNA_rep2)
...
bc_df <- bc_df[bc_df$n_bc >= 300 | bc_df$condition == "pMT06_pDNA_rep1" | bc_df$condition == "pMT06_pDNA_rep2",] %>% dplyr::select(-n_bc)

# Remove samples with high pDNA contamination
bc_df <- bc_df[bc_df$cor <= 0.8 | bc_df$condition == "pMT06_pDNA_rep1" | bc_df$condition == "pMT06_pDNA_rep2",] %>% dplyr::select(-cor)
...
pDNA <- bc_df[grep("pDNA", bc_df$condition),] %>% dplyr::select(barcode, condition, rpm) %>% unique() %>%
  ...
  mutate(pDNA_counts_rpm = (pMT06_pDNA_rep1 + pMT06_pDNA_rep2) / 2) %>%
...
bc_df$activity[bc_df$pDNA_counts_rpm <= 50] <- NA
```
...
bc_df_rep <- bc_df_cDNA_filt[bc_df_cDNA_filt$rand_promoter == "No",] %>% 
  ...
  mutate(pDNA_counts_rpm = ave(pDNA_counts_rpm, reporter_id, FUN = function(x) mean(x))) %>% unique()
...
cor_df <- data.frame("rpm_cutoff"  = seq(0,100,1), "cor" = "")
...
for (i in unique(cor_df$rpm_cutoff)) {
  for (j in unique(cor_df$x)) {
    ...
    if (nrow(x) == 0) {
      ...
      cor_df$cor[cor_df$rpm_cutoff > i & cor_df$x == j] <- cor(x$bc2, x$bc4, "pairwise.complete.obs")
    }
  }
}
...
bc_df_rep_2 <- bc_df_rep[bc_df_rep$pDNA_counts_rpm >= 100,]
